function [x, u, L, kFclose, KFclose, xFclose, ...
          Vx, Vxx, cost, trace, stop] = iLQGSecond(DYNCST, x0, u0, ...
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost,kFclose,KFclose,xFclose] = forward_pass_second(x0(:,1),alpha*u,[],[],[], ...
                                                                  k_First,K_First,xn_First, ...
...
elseif size(x0,2) == N+1 % pre-rolled initial forward pass
    ...
    if isempty(Op.cost)
        ...
        xFclose  = zeros(size(xn_First, 1), N+1, 1);
        for ii=1:N+1
            [~,closest_x]       = min(vecnorm(x0(:, ii) - xn_First));
            kFclose(:, ii, 1)   = k_First(:, closest_x);
            KFclose(:,:, ii, 1) = K_First(:,:, closest_x);
            xFclose(:, ii, 1)   = xn_First(:, closest_x);
        end
    end
else
...
for iter = 1:Op.maxIter
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew,...
             kFclosenew,KFclosenew,xFclosenew] = forward_pass_second(x0 ,u, L, x(:,1:N), l, ...
                                                            k_First, K_First, xn_First, ...
            ...
            for alpha = Op.Alpha
                [xnew,unew,costnew,...
                 kFclosenew,KFclosenew,xFclosenew]   = forward_pass_second(x0 ,u+l*alpha, L, x(:,1:N),[], ...
                                                                  k_First, K_First, xn_First, ...

...
for jj=1:1:size(x_starts, 2)
    ...
    [XCartTS(:,:, jj), ...
     ...
     ~, ~, CCartTS(:,:, jj), TraceCartTS{jj,1}] = iLQGSecondKDTree(dyn_CartTS, ...
                                        x_starts(:, jj), ...
                                        Uinit, ...
                                        UPoleFF(:,:, mod(jj-1, size(pole_starts,2))+1), ...
                                        KPoleFF(:,:,:, mod(jj-1, size(pole_starts,2))+1), ...
                                        XPoleFF(sys_PoleFF.X_DIMS_FREE,:,mod(jj-1, size(pole_starts,2))+1), ...
                                        Op);
    timeCartTS(jj) = toc;
    jj
end
...
status = "zeroinit_finallpoint_onlycostwraparound_difffurthercloserstarts_lowlimits_finestalpha"

save(strcat('data/iLQGCartPoleDecomposedOnlyCostWrapAround_diffR_difffurthercloserstarts_lowlimits_alpha_',num2str(Op.Alpha),'_mc=',num2str(sys.mc),',mp=',num2str(sys.mp),'.mat'));
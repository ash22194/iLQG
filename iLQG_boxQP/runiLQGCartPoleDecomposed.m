...
for jj=1:1:size(x_starts, 2)
    ...
    [XJoint(sys_joint.X_DIMS_FREE,:, jj), ...
     ...
     ~, ~, CJoint(:,:, jj), Trace] = iLQG(dyn_joint, ...
...
status = 'zeroinit_finallpoint_nowraparound_furthercloserstarts_coarsergamma';

save(strcat('data/iLQGCartPoleDecomposed_diffR_furthercloserstarts_gamma_',num2str(sys.gamma_),'mc=',num2str(sys.mc),',mp=',num2str(sys.mp),'.mat'));

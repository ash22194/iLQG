function all_possible_children = enumerate_possible_children(de)
    ...
    X_DIMS = sum(cellfun(@(x) sum(x), de.action_tree(:,2)));
    ...
    for jj=1:1:size(de.action_tree,1)
        ...
        parent_node = cellfun(@(x) setequal(x,parent_node), action_tree_(:,1));
        curr_child_node = cellfun(@(x) setequal(x, inputs), action_tree_{parent_node, end});
            ...
            for nc=1:1:num_children_groups
                ...
                for ns=1:1:num_state_groups
                    ...
                    if ~(((num_ch1==0) && (isempty(curr_state_group{1}))) ...
                        ...
                        all_possible_children{children_count+1} = policy_decomposition(de.sys, action_tree_, de);

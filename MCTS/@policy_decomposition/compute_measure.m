function measure = compute_measure(de)
    ...
    if ((de.lqr_measure_table.isKey(de.decomposition_key)) ...
        && (de.compute_fraction_table.isKey(de.decomposition_key)) ...
        && (de.measure_table.isKey(de.decomposition_key)))
        
        de.lqr_measure = de.lqr_measure_table(de.decomposition_key);
        de.compute_fraction = de.compute_fraction_table(de.decomposition_key);
        de.measure = de.measure_table(de.decomposition_key);
        measure = de.measure;
        return;
    end
    ...
    while (~isempty(action_tree_))
        ...
        if (size(leaf_nodes, 1)==1 && all(leaf_nodes{1,1}==0))
            ...
            de.lqr_measure_table(de.decomposition_key) = err_lqr;

            de.compute_fraction = err_compute;
            de.compute_fraction_table(de.decomposition_key) = err_compute;

            de.measure = measure;
            de.measure_table(de.decomposition_key) = measure;
            return;
        end

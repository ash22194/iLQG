...
%% 
iLQG_dir = 'data/';
iLQG_filename = 'iLQGCartPoleDecomposed_mc=5,mp=1.mat';
DP_cont_dir = 'data/';
DP_cont_filename = 'mc=5,mp=1_wraparound.mat';
...
[trajXJoint, trajUJoint, J_DP_rollout(:,1)] = rolloutDPTraj(policy_joint, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyCartFPoleT(:,:,:,:,1) = f_cart_first_;
policyCartFPoleT(:,:,:,:,2) = t_pole_second;
[trajXCFPT, trajUCFPT, J_DP_rollout(:,2)] = rolloutDPTraj(policyCartFPoleT, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyPoleTCartF(:,:,:,:,2) = t_pole_first_;
policyPoleTCartF(:,:,:,:,1) = f_cart_second;
[trajXPTCF, trajUPTCF, J_DP_rollout(:,3)] = rolloutDPTraj(policyPoleTCartF, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyCartTPoleF(:,:,:,:,2) = t_cart_first_;
policyCartTPoleF(:,:,:,:,1) = f_pole_second;
[trajXCTPF, trajUCTPF, J_DP_rollout(:,4)] = rolloutDPTraj(policyCartTPoleF, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyPoleFCartT(:,:,:,:,1) = f_pole_first_;
policyPoleFCartT(:,:,:,:,2) = t_cart_second;
[trajXPFCT, trajUPFCT, J_DP_rollout(:,5)] = rolloutDPTraj(policyPoleFCartT, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyCartFPoleTDec(:,:,:,:,1) = f_cart_first_;
policyCartFPoleTDec(:,:,:,:,2) = t_pole_first_;
[trajXCFPTD, trajUCFPTD, J_DP_rollout(:,6)] = rolloutDPTraj(policyCartFPoleTDec, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);

policyCartTPoleFDec(:,:,:,:,1) = f_pole_first_;
policyCartTPoleFDec(:,:,:,:,2) = t_cart_first_;
[trajXCTPFD, trajUCTPFD, J_DP_rollout(:,7)] = rolloutDPTraj(policyCartTPoleFDec, x_starts, sys, limits, grid_x, grid_x_dot, grid_xP, grid_xP_dot);
...
for jj=1:1:size(x_starts, 2)
    ...
    plot(XJoint(1,:,jj),XJoint(2,:,jj),'Color',[0.4660 0.6740 0.1880], 'LineWidth', 5);
    plot(XPoleTS(1,:,jj),XPoleTS(2,:,jj),'Color',[0 0.4470 0.7410], 'LineWidth', 5);
    plot(XCartFS(1,:,jj),XCartFS(2,:,jj),'Color',[0.8500 0.3250 0.0980], 'LineWidth', 5);
    plot(XPoleFS(1,:,jj),XPoleFS(2,:,jj),'Color',[0.9290 0.6940 0.1250], 'LineWidth', 5);
    plot(XCartTS(1,:,jj),XCartTS(2,:,jj),'Color',[0.4940 0.1840 0.5560], 'LineWidth', 5);
    plot(XCartFPoleTDec(1,:,jj), XCartFPoleTDec(2,:,jj), 'Color', [0.3010 0.7450 0.9330], 'LineWidth', 5);
    plot(XCartTPoleFDec(1,:,jj), XCartTPoleFDec(2,:,jj), 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 5);
    % plot DP trajectories
    plot(trajXJoint(1,:,jj), trajXJoint(2,:,jj),'Color',[0.4660 0.6740 0.1880], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCFPT(1,:,jj), trajXCFPT(2,:,jj),'Color',[0 0.4470 0.7410], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXPTCF(1,:,jj), trajXPTCF(2,:,jj),'Color',[0.8500 0.3250 0.0980], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCTPF(1,:,jj), trajXCTPF(2,:,jj),'Color',[0.9290 0.6940 0.1250], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXPFCT(1,:,jj), trajXPFCT(2,:,jj),'Color',[0.4940 0.1840 0.5560], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCFPTD(1,:,jj), trajXCFPTD(2,:,jj), 'Color', [0.3010 0.7450 0.9330], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCTPFD(1,:,jj), trajXCTPFD(2,:,jj), 'Color', [0.6350 0.0780 0.1840], 'LineStyle', '-.', 'LineWidth', 5);
    ...
    hold on;
    % plot DDP trajectories
    plot(XJoint(3,:,jj), XJoint(4,:,jj),'Color',[0.4660 0.6740 0.1880], 'LineWidth', 5);
    plot(XPoleTS(3,:,jj), XPoleTS(4,:,jj),'Color',[0 0.4470 0.7410], 'LineWidth', 5);
    plot(XCartFS(3,:,jj), XCartFS(4,:,jj),'Color',[0.8500 0.3250 0.0980], 'LineWidth', 5);
    plot(XPoleFS(3,:,jj), XPoleFS(4,:,jj),'Color',[0.9290 0.6940 0.1250], 'LineWidth', 5);
    plot(XCartTS(3,:,jj), XCartTS(4,:,jj),'Color',[0.4940 0.1840 0.5560], 'LineWidth', 5);
    plot(XCartFPoleTDec(3,:,jj), XCartFPoleTDec(4,:,jj), 'Color', [0.3010 0.7450 0.9330], 'LineWidth', 5);
    plot(XCartTPoleFDec(3,:,jj), XCartTPoleFDec(4,:,jj), 'Color', [0.6350 0.0780 0.1840], 'LineWidth', 5);
    % plot DP trajectories
    plot(trajXJoint(3,:,jj), trajXJoint(4,:,jj),'Color',[0.4660 0.6740 0.1880], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCFPT(3,:,jj), trajXCFPT(4,:,jj),'Color',[0 0.4470 0.7410], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXPTCF(3,:,jj), trajXPTCF(4,:,jj),'Color',[0.8500 0.3250 0.0980], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCTPF(3,:,jj), trajXCTPF(4,:,jj),'Color',[0.9290 0.6940 0.1250], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXPFCT(3,:,jj), trajXPFCT(4,:,jj),'Color',[0.4940 0.1840 0.5560], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCFPTD(3,:,jj), trajXCFPTD(4,:,jj), 'Color', [0.3010 0.7450 0.9330], 'LineStyle', '-.', 'LineWidth', 5);
    plot(trajXCTPFD(3,:,jj), trajXCTPFD(4,:,jj), 'Color', [0.6350 0.0780 0.1840], 'LineStyle', '-.', 'LineWidth', 5);

function err_lqr = computeLQRMeasureDecoupledwPseudoInputs(sys, s)
%% Inputs
...
if (any(c_eq~=0) || any(c > 0))
    ...
    [sC, sia, sic] = unique(s, 'rows');
    U_PSEUDO_DIMS = cell(length(sia), 1);
    for jj=1:1:sys.U_DIMS
        U_PSEUDO_DIMS{sic(jj),1} = [U_PSEUDO_DIMS{sic(jj),1}; jj];
    end
    ...
    %     B_ = sys.B(s(ii,:), ii);
        u0_ = u0;
        u0_(U_PSEUDO_DIMS{ii}) = sys.u0(U_PSEUDO_DIMS{ii});
        fxu = eval(subs(sys.fxu, [sys.x; sys.u], [sys.l_point; u0_]));
        A_ = fxu(:,1:sys.X_DIMS);
        A_ = A_(sC(ii,:), sC(ii,:));
        B_ = sys.B(sC(ii,:), U_PSEUDO_DIMS{ii}); % B calculation remains the same as system is control affine

        Q_ = sys.Q(sC(ii,:), sC(ii,:));
        R_ = sys.R(U_PSEUDO_DIMS{ii}, U_PSEUDO_DIMS{ii});

        [K(U_PSEUDO_DIMS{ii}, sC(ii,:)), ~, ~] = lqr(A_ - eye(size(A_,1))*sys.lambda_/2, B_, ...

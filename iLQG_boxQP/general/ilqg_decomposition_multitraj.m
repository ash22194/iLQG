...
%                 K_ = zeros(size(B_,2), size(A_,1));
            ...
            if (isfield(Op, 'reuse_policy') && Op.reuse_policy && isfile(strcat('data/iLQGGeneral', sys.name, '/decomp', num2str(sys.decomposition_id), '.mat')))
                decomposition = load(strcat('data/iLQGGeneral', sys.name, '/decomp', num2str(sys.decomposition_id), '.mat'));

function [x, u, L, kFclose, KFclose, xFclose, ...
          Vx, Vxx, cost, trace, stop] = iLQGSecondKDTree(DYNCST, x0, u0, ...
                                                   k_First, K_First, xn_First, ...
                                                   Op)
...
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost,kFclose,KFclose,xFclose] = forward_pass_second_kdtree(x0(:,1),alpha*u,[],[],[], ...
                                                                      k_First,K_First,xn_First,Mdl,Op.X_DIMS_FIRST, ...
                                                                  1,Op.gamma_,DYNCST,Op.lims,[]);
...
elseif size(x0,2) == N+1 % pre-rolled initial forward pass
    ...
    if isempty(Op.cost)
        ...
        cost     = Op.cost;
        kFclose  = zeros(size(k_First, 1), N+1, 1);
        KFclose  = zeros(size(K_First, 1), size(K_First, 2), N+1, 1);
        xFclose  = zeros(n, N+1, 1);
%         closest_x = knnsearch(Mdl, x0(Op.X_DIMS_FIRST,:)');
        [ closest_x, ~, Mdl ] = kdtreeidx([], x0(Op.X_DIMS_FIRST,:)', ...
                                                     Mdl);
        
        kFclose(:, :, 1)   = k_First(:, closest_x);
        KFclose(:,:, :, 1) = K_First(:,:, closest_x);
        xFclose(Op.X_DIMS_FIRST, :, 1)   = xn_First(:, closest_x);
...
for iter = 1:Op.maxIter
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew,...
             kFclosenew,KFclosenew,xFclosenew] = forward_pass_second_kdtree(x0 ,u, L, x(:,1:N), l, ...
                                                            k_First, K_First, xn_First, Mdl, Op.X_DIMS_FIRST, ...
                                                            Op.Alpha, Op.gamma_, DYNCST,Op.lims,Op.diffFn);
            ...
            if (z > Op.zMin) 
                ...
                xnew        = xnew(:,:,w);
                unew        = unew(:,:,w);
                kFclosenew  = kFclosenew(:,:,w);
                KFclosenew  = KFclosenew(:,:,:,w);
                xFclosenew  = xFclosenew(:,:,w);
            end
        else            % serial backtracking line-search
            for alpha = Op.Alpha
                [xnew,unew,costnew,...
                 kFclosenew,KFclosenew,xFclosenew]   = forward_pass_second_kdtree(x0 ,u+l*alpha, L, x(:,1:N),[], ...
                                                                  k_First, K_First, xn_First, Mdl, Op.X_DIMS_FIRST, ...
                                                                  1,Op.gamma_,DYNCST,Op.lims,Op.diffFn);

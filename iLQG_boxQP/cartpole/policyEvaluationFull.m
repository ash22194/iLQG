function G = policyEvaluationFull(mc, mp, l, g, goal, dt, Q, R, gamma_, policyF, policyT, grid_x, grid_x_dot, grid_xP, grid_xP_dot, gtol, max_iter, varargin)
    ...
    goal_grid = [0, 0, 0, 0];
    [~, goal_grid(1)] = min(abs(reshape(grid_x(:,1,1,1), [size(grid_x, 1), 1]) - goal(1)));
    [~, goal_grid(2)] = min(abs(reshape(grid_x_dot(1,:,1,1), [size(grid_x, 2), 1]) - goal(2)));
    [~, goal_grid(3)] = min(abs(reshape(grid_xP(1,1,:,1), [size(grid_x, 3), 1]) - goal(3)));
    [~, goal_grid(4)] = min(abs(reshape(grid_xP_dot(1,1,1,:), [size(grid_x, 4), 1]) - goal(4)));

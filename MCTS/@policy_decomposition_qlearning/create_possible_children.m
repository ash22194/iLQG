function all_possible_children = create_possible_children(de, predictor)
...
%     all_possible_children_encoding = zeros(1, (U_DIMS + X_DIMS)^2, 1, de.childnode_maxcount);
    de.childnodes_unexpanded = true(de.childnode_maxcount,1);
    de.childnodes_subtree_unexplored = true(de.childnode_maxcount, 1);
    children_count = 0;
                ...
                for ns=1:1:num_state_groups
                    ...
                    if ~(((num_ch1==0) && (isempty(curr_state_group{1}))) ...
                        ...
                        if (de.nodelist.isKey(action_tree_encoding_str))
                            ...
                            if (~all_possible_children{children_count+1}.subtree_unexplored)
                                de.childnodes_subtree_unexplored(children_count+1) = false;
                            end
                        else
...
%                         [A_, B_] = pre_process_dyn(de.sys, action_tree_);
%                         all_possible_children_encoding(1,:,1,children_count+1) = [action_tree_encoding, ...
%                                                                                   reshape(A_, 1, X_DIMS*X_DIMS), ...
%                                                                                   reshape(B_, 1, U_DIMS*X_DIMS)];

    ...
    int nrhs, mxArray const* prhs[])
{
    ...
    active_actions = mxGPUCreateFromMxArray(prhs[12]);

    p_m = mxGetDoubles(prhs[6]); 
    double const mc = p_m[0];
    p_m = mxGetDoubles(prhs[7]); 
    double const mp = p_m[0];

    p_l = mxGetDoubles(prhs[8]); 
    double const l = p_l[0];

    p_g = mxGetDoubles(prhs[9]);
    double const g = p_g[0];
    
    p_dt = mxGetDoubles(prhs[10]);
    double const dt = p_dt[0];
    
    p_limits = mxGetDoubles(prhs[13]);

function [xnew,unew,ulast,cnew,sub_trajectories_close] = forward_pass_general_kdtree(x0,u,L,x,du,...
...
sub_trajectories_close_i = cell(size(sub_trajectories, 1), size(sub_trajectories, 2));
...
for i = 1:N
    ...
    for jj=1:1:size(sub_trajectories, 1) 
        closest_xn2 = knnsearch(Mdl{jj}, xnew(sub_trajectories{jj, 2}, :, i)');
...
for jj=1:1:size(sub_trajectories, 1)
    closest_xn2 = knnsearch(Mdl{jj}, xnew(sub_trajectories{jj, 2}, :, N+1)');
    sub_trajectories_close_i{jj, 3} = sub_trajectories{jj, 3}(:, closest_xn2);
    sub_trajectories_close_i{jj, 4} = sub_trajectories{jj, 4}(:,:, closest_xn2);
    sub_trajectories_close_i{jj, 5} = sub_trajectories{jj, 5}(:, closest_xn2);

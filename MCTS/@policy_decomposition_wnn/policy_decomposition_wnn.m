classdef policy_decomposition_wnn < handle
    properties
        sys
        p
        s
    ...
    methods 
        ...
        function de = policy_decomposition_wnn(varargin)
            %% Description
            % Each node stores a list of child nodes that are objects of
            % type policy_decomposition_wnn when expanded and string
            ...
            % encodings as keys
            % A trained neural network is used to decide which unexpanded
            % nodes to expand
            ...
            de.decomposition_key = fastint2str(encode_binary_adj(de.action_tree));
            de.parent = parent;
            if (isa(parent, 'policy_decomposition_wnn'))
        ...
        function child = expand_best_child(de, determ)
            ...
            childnodes_unexpanded = cellfun(@(x) ~isa(x, 'policy_decomposition_wnn'), de.childnodes);
            if (any(childnodes_unexpanded))
                childnodes_unexplored_fully = de.childnodes(childnodes_unexpanded);
                childnodes_unexplored_measure = de.childnodes_elqr_prediction(childnodes_unexpanded);
                [~, max_uctchild] = max(-childnodes_unexplored_measure);
%                 max_uctchild = 1;
%                 if (~determ)
...
%                 end
                ...
                if (de.nodelist.isKey(action_tree_encoding))
                    ...
                    child = policy_decomposition_wnn(de.sys, ...
                                                 action_tree_, ...
                                                 de);
                end
